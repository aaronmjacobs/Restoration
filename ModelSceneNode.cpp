...
ModelSceneNodeRef ModelSceneNode::fromFile(SceneGraph *graph, const std::string &name, const std::string &fileName, ShaderProgram *program) {
   ...
   return ModelSceneNodeRef(new ModelSceneNode(graph, name, vao, numIndices));
}

ModelSceneNode::ModelSceneNode(SceneGraph *graph, const std::string &name, const GLuint vao, const unsigned int numIndices)
   : SceneNode(graph, name), vao(vao), numIndices(numIndices) {

}

void ModelSceneNode::draw() {
   graph->getMatrixStack()->push();

   glBindVertexArray(vao);
   glDrawElements(GL_TRIANGLES, numIndices, GL_UNSIGNED_INT, 0);
   glBindVertexArray(0);

   graph->getMatrixStack()->pop();
}

void ModelSceneNode::tick() {
